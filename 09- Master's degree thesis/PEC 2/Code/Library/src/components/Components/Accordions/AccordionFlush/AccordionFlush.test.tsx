...
import AccordionFlush, { AccordionFlushProps } from ".";

const props: AccordionFlushProps = {
  data: [
    {
...
describe("<Accordion />", () => {
  ...
  it("should open the first item", () => {
    const { container } = render(<AccordionFlush {...props} />);
    const firstItem = queryByTestId(container, "accordion-item-btn-1");
    if (firstItem) {
      fireEvent.click(firstItem);
      expect(firstItem.className).toContain("text-gray-700");
    } else {
